    ...
    <div>
        <div class="form-group mb-3" v-for="(item, index) in items">
            <div class="row">
                <div class="col-md-3 col-sm-5">
                    <div class="form-group mb-3">
                        <div class="ui-select-wrapper">
                            <select :name="'facilities[' + index + 1 + '][id]'"
                                    class="select-search-full ui-select" @change="removeSelectedItem">
                                <option value="">{{ __('select_facility') }}</option>
                                <option
                                    v-for="(facility, index) in facilities" :key="index" :value="facility.id" :selected="facility.id === item.id">{{ facility.name }}
                                </option>
                            </select>
                ...
                <div class="col-md-3 col-sm-5">
                    <div class="form-group mb-3">
                        <input type="text" :name="'facilities[' + index + 1 + '][distance]'" v-model="item.distance" class="form-control"
